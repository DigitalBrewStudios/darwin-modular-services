...
let
  ...
  makeEtcFiles =
    ...
    let
      ...
      subEntries = concatMapAttrs (n: sub: makeEtcFiles (translate.dash prefix n) sub) (
        service.services or { }
      );
    in
...
in
{
  ...
  config = {
    launchd.daemons = concatMapAttrs (n: service: daemonEntries n service) config.system.services;

    environment.etc = concatMapAttrs (n: service: makeEtcFiles n service) config.system.services;
