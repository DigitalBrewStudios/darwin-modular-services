...
let
  ...
  agentsForUser =
    userName: services: concatMapAttrs (n: service: agentEntries userName n service) services;
    ...
    let
      collect =
        ...
        let
          ...
          subs = concatMapAttrs (n: sub: collect (translate.dash prefix n) sub) (service.services or { });
        in
        own // subs;
  ...
  activationSnippet = lib.concatStringsSep "\n" (
    lib.mapAttrsToList (
      userName: user:
      let
  ...
  config = {
    ...
    warnings = lib.concatLists (
      lib.mapAttrsToList (
        ...
        lib.concatLists (
          lib.mapAttrsToList (
            n: service: (servicesLibFor userName).getWarnings [ "users" "users" userName "services" n ] service
          ) user.services
        )
      ) usersWithServices
    );

    system.activationScripts.userLaunchd.text = lib.mkIf (usersWithServices != { }) (
      lib.mkAfter activationSnippet
    );
  };
}
